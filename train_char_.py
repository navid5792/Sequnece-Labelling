...
import pickle
with open("pretrained_dict_100G.pkl" , "rb") as f:
...
#encoder.load_state_dict(torch.load('best_model_97.16'))
#crf.load_state_dict(torch.load('best_crf_97.16'))
...
#        input_batches_prefix = Variable(torch.LongTensor(prefix_seqs), volatile=True).transpose(0, 1)
        ...
        for i in range(input_batches.size(0)):  # on sequence
            ...
            for j in range(input_batches.size(1)): # on batch
                bat.append(return_mask(input_lang.index2word[input_batches[i][j].data.cpu().numpy()[0] ], i ,input_lengths[j]))
        ...
        for i in range(input_batches.size(0)):  # on sequence
            bat = []
            for j in range(input_batches.size(1)): # on batch
                bat.append(return_lastTwo_mask(input_lang.index2word[input_batches[i][j].data.cpu().numpy()[0] ]))
            last_two.append(bat)
        ...
        for i in range(input_batches.size(0)):  # on sequence
            bat = []
            for j in range(input_batches.size(1)): # on batch
                bat.append(word2suffix[input_lang.index2word[input_batches[i][j].data.cpu().numpy()[0] ]])
            suffix.append(bat)
        ...
        if USE_CUDA:
            ...
            input_batches_sense = input_batches_sense.cuda()
            input_batches_spelling = spellings.cuda()
            last_two = last_two.cuda()
            suffix = suffix.cuda()
...
#        temp_right_actual = []
        if (input_lengths[0]) > 0:
            index = encoder(input_batches, input_lengths, input_batches_sense, None, suffix, input_batches_spelling, last_two,  None, None, train = False).transpose(0,1)
...
#    print("training")
    ...
    lens_s = Variable(torch.cuda.LongTensor(input_lengths))
...
while epoch < n_epochs:
    print("....." , epoch)
    epoch += 1
    print("dropout 0.5 and 0.1 (embedding), clip grad 2.5, LR 0.015 SGD last two nai and w1 w2 w3 suffix (BINARY) LSTM er input e glove 100")
    for index in range(no_of_batches + 1):
    ...
    # Get training data for this cycle
        ...
        spellings = Variable(torch.FloatTensor(spellings)).cuda()
        ...
        last_two = Variable(torch.FloatTensor(last_two)).cuda()
        ...
        suffix = Variable(torch.FloatTensor(suffix)).cuda()
...
#                pickle.dump([F1_score,ACC_seq],f)
    ...
    optimized_evaluate(test_pairs,0)
#    print("W1 = ", encoder.W1, " W2 = ", encoder.W2)
    print("W1 = ", encoder.W1, " W2 = ", encoder.W2, " W3 = ", encoder.W3)
...
encoder.load_state_dict(torch.load('best_model'))
crf.load_state_dict(torch.load('best_crf'))
