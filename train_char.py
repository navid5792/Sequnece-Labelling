...
def optimized_evaluate(test_pairs,check):
    ...
    for j in range(len(indexes)):    
        ...
        target_batches = torch.LongTensor(target_seqs).transpose(0,1) 
        
        input_seqs_sense = [indexes_from_sentence(input_lang_sense, input_sentence_sense)]
        input_batches_sense = torch.LongTensor(input_seqs_sense).transpose(0, 1)
        ...
        spellings = torch.FloatTensor(spellings)
        ##### last two
        last_two = []
        for i in range(input_batches.size(0)):  # on sequence
            bat = []
        ...
        last_two = torch.FloatTensor(last_two)
        ...
        suffix = torch.FloatTensor(suffix)
...
def train(input_batches, input_lengths, target_batches, target_lengths, input_batches_sense, spellings, last_two, suffix,  encoder,   encoder_optimizer,   criterion, max_length=5000):
    ...
    lens_s = torch.tensor(input_lengths).cuda()
...
while epoch < n_epochs:
    ...
    # Get training data for this cycle
        ...
        spellings =torch.FloatTensor(spellings).cuda()       
        ...
        last_two = torch.tensor(last_two).cuda()
        ...
        suffix = torch.FloatTensor(suffix).cuda()
